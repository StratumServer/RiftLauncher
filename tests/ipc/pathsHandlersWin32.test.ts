...
describe("RUN_INSTALLER on win32: payload extraction", () => {
  ...
  it("still resolves ok, but logs a warning, when the payload reader reports a cleanup failure alongside extracted (#528)", async () => {
    ...
    const worker = await workerPromise
    worker.emit("message", { type: "finished", verdict: "extracted", filesWritten: 12, bytesWritten: 4096, cleanupWarning: "installer-cleanup-failed" })
    ...
    assert.equal(
      warnLines.some((line) => line.includes("[RUN_INSTALLER]") && line.includes("reason=installer-cleanup-failed")),
      true,
