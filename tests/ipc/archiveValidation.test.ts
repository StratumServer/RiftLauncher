...
describe("validateArchive", () => {
  it("accepts a gzipped tar, which is what the Linux game builds ship as", async () => {
    ...
    await validateArchive(archivePath, path7za)
  })
  ...
  it("refuses a gzipped tar carrying a symbolic link", async () => {
    ...
    await assert.rejects(validateArchive(archivePath, path7za), /unsafe entry/)
  })
  ...
  it("refuses a file that only claims to be a gzipped tar", async () => {
    ...
    await assert.rejects(validateArchive(workspacePath("liar.tar.gz"), path7za), /could not be read/)
  })

  it("still reads a zip with the zip reader", async () => {
    const archivePath = workspacePath("backup.zip")
    execFileSync(path7za, ["a", "-tzip", archivePath, workspacePath("source", "vintagestory")], { stdio: "ignore" })
  ...
  it("refuses a zip that cannot be parsed", async () => {
    ...
    await assert.rejects(validateArchive(workspacePath("broken.zip"), path7za), /could not be read/)
  })

  it("falls back to 7-Zip for anything else", async () => {
    const archivePath = workspacePath("backup.7z")
    execFileSync(path7za, ["a", "-t7z", archivePath, workspacePath("source", "vintagestory")], { stdio: "ignore" })

    await validateArchive(archivePath, path7za)
    await assert.rejects(validateArchive(workspacePath("source", "vintagestory", "Vintagestory"), path7za), /could not be listed/)
  })
})
