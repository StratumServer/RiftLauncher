 ...
 *
 * The file is read as text, the same way electron-builder-locales.test.ts does: js-yaml is only a
 * transitive dependency of electron-updater, and importing it here would be an undeclared one.
 */

const YML = readFileSync(resolve(__dirname, "../../electron-builder.yml"), "utf8")

/** The list under `key:` inside the `section:` block, read off the indentation the file uses. */
function listUnder(section: string, key: string): string[] {
  const block = YML.split(new RegExp(`^${section}:$`, "m"))[1] ?? ""
  const afterKey = block.split(new RegExp(`^  ${key}:$`, "m"))[1] ?? ""
  const entries: string[] = []
  for (const line of afterKey.split("\n").slice(1)) {
    const value = /^ {4}-\s+(.+?)\s*$/.exec(line)?.[1]
...
describe("electron-builder Linux package dependencies", () => {
  it("keeps every deb dependency, libsecret included", () => {
    assert.deepEqual(listUnder("deb", "depends"), ["libgtk-3-0", "libnotify4", "libnss3", "libxss1", "libxtst6", "xdg-utils", "libatspi2.0-0", "libuuid1", "libsecret-1-0"])
  })
  ...
  it("recommends a keyring daemon on deb without requiring one, and keeps the default recommends beside it", () => {
    ...
    assert.deepEqual(listUnder("deb", "recommends"), ["libappindicator3-1", "gnome-keyring"])
  })

  it("keeps every rpm dependency, libsecret included", () => {
    assert.deepEqual(listUnder("rpm", "depends"), ["gtk3", "libnotify", "nss", "libXScrnSaver", "(libXtst or libXtst6)", "xdg-utils", "at-spi2-core", "(libuuid or libuuid1)", "libsecret"])
  })
