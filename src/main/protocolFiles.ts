...
export function createCacheModImageProtocolHandler({ cache, getUserDataPath, fetchFile }: CacheModImageProtocolPorts): (request: Request) => Promise<Response> {
  return async (request) => {
    ...
    try {
      const revision = await getProtocolFileRevision(filePath)
      if (revision === undefined) return new Response(null, { status: 404 })
      const bytes = await readIconWithMemoryCache(
        cache,
        filePath,
        async () => {
 ...
 * deliberately missing is the memory cache. Archive icon paths are content-addressed by their byte
 * hash, while URL-keyed ModDB paths may be refreshed and are revision-checked by the mod image
 * handler. The custom background keeps one stable name whose bytes are replaced whenever the player
