 ...
 */
ipcMain.handle(IPC_CHANNELS.BACKGROUNDS_MANAGER.ENSURE_BACKGROUND, async (event, id: unknown, file: unknown, expectedSha256: unknown): Promise<boolean> => {
  ...
  try {
    ...
    if (await isCurrentCachedBackground(id, expectedHash)) return true
