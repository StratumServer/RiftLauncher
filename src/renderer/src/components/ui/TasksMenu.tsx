...
import { BUTTON_BASE_STYLES, BUTTON_VARIANT_STYLES, BUTTON_SIZE_STYLES } from "@renderer/components/ui/buttonStyles"
...
function TasksMenu(): JSX.Element {
  ...
  return (
    <Popover className="relative">
      {({ open }) => (
        <>
          <PopoverButton title={t("components.tasksMenu.title")} className={clsx(BUTTON_BASE_STYLES, BUTTON_SIZE_STYLES.sm, BUTTON_VARIANT_STYLES.secondary, "w-8 h-8 aspect-square overflow-hidden")}>
            <PiDownloadDuotone />
          </PopoverButton>
                          ...
                          <motion.li key={task.id} variants={DROPDOWN_MENU_ITEM_VARIANTS} className="w-full flex flex-col odd:bg-zinc-800/30 even:bg-zinc-950/30">
                            <div className="w-full flex justify-between gap-2 p-1">
                              ...
                              {(task.status === "completed" || task.status === "failed") && (
                                <NormalButton className="p-1 text-zinc-400" title={t("generic.discard")} variant="ghost" onClick={() => removeTask(task.id)}>
                                  <PiXCircleDuotone />
                                </NormalButton>
                              )}
                            </div>
