  ...
  return {
    ...
    downloader: {
      download: (request, onComplete) =>
        startDownload(taskName, downloadDescription, "caller-handled", request.url, request.outputFolder, request.fileName, (status, path, error) =>
    ...
    unpacker: {
      ...
      extractArchive: (request, onComplete) =>
        startExtract(taskName, unpackDescription, "caller-handled", request.sourcePath, request.outputFolder, true, (status, error) => onComplete({ ok: status, error: error?.message }), true),
      runInstaller: (request, onComplete) =>
        startInstall(taskName, unpackDescription, "caller-handled", request.sourcePath, request.outputFolder, true, (status, error) => onComplete({ ok: status, error: error?.message }))
    }
  }
}
