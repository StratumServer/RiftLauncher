...
Five CI jobs, defined in `.github/workflows/ci.yml`, run on every PR: `typecheck`, `lint` (`lint:ci` + `format:check`), `test` (`test:coverage`), `sonarcloud` (informational, `continue-on-error: true`), and `build` (`build:unpack`, on Ubuntu and Windows). A sixth, `build-macos`, is `workflow_dispatch`-only.
